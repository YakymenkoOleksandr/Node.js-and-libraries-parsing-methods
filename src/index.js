...
app.use((req, res, next) => {
  // В Node.js передача "*" більше не працює, тепер передаютьсятульки req, res i next.
  res.status(404).json({
    // Дана мідлвара вказує про некоректність запиту
    message: 'Є брать там нікого нет.',
  });
});

app.use((err, req, res, next) => {
  // Мідлвара ерор перехоплювач. Мість аргументи в колбекі req, res, next, error що характерно тількия для ерормідлвар.
  res.status(500).json({
    message: 'Ля він упав',
    error: err.message,
  });
...
app.use( // Мідлвара з бібліотеки pino-http, для читабельного форматування відповідей в консолі.
  pino({ // pino генерує JSON-логи.
    transport: { // transport — спосіб перенаправити потік логів у сторонній "вивід"
      target: 'pino-pretty', // target — назва модуля, який обробляє вивід логів. pino-pretty перетворює їх у зручний формат.
    },
  }),
);
